; ---------------------------------------------------------------------------
;  LYGO Local Agent Console - PC LOCAL - installer, 1.5.3
;
;  Payload : the SEALED 1.5.3 build for this machine
;            D:\LYGO_CANON\2026-09-24_build-1.5.3_PC_LOCAL\PC_LOCAL
...
;            For the installer with the weights INSIDE, use LYGO_LLM_CONSOLE_1.5.3_PC_SETUP_FULL.
...
AppComments=One console, three systems: USB LOCAL / PC LOCAL / WEB PORTAL (API only). Sealed 1.5.3 build, full engine included, and the picture engine now travels INSIDE the package: a fresh install draws a picture with no config at all once the checkpoint is fetched (one file, 6.9 GB, fetched from its author, not redistributed by us). 1.5.3 adds the music systems: the exact YuE-shaped lyric template is the box's default (one press restores it), the sheet is validated and converted without spending a render, and the agent can write a whole song into the box from a brief or at random from the engine's own tag vocabulary.
AppCopyright=LYGO Sovereign License v3.0 - no resale, no rebranding, no modified redistribution.
VersionInfoVersion=1.5.3.0
VersionInfoDescription=LYGO Local Agent Console (PC LOCAL) 1.5.3 - sealed build, chat engine + picture engine included, weights fetched from our own vault
...
OutputDir=D:\
OutputBaseFilename=LYGO_LLM_CONSOLE_1.5.3_PC_SETUP
...
begin
  if CurStep = ssPostInstall then
    Log('LYGO PC LOCAL ' + '{#Release}' + ' installed from the sealed 1.5.3 build to ' + ExpandConstant('{app}') +
