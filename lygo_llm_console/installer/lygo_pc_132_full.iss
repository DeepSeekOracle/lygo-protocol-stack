; ---------------------------------------------------------------------------
;  LYGO Local Agent Console - PC LOCAL - FULL installer, 1.3.2  (everything inside)
;
;  Payload : the SEALED 1.3.2 build  D:\LYGO_CANON\2026-09-24_build-1.3.2_PC_LOCAL\PC_LOCAL
...
;            <install>\models\sd, with its CreativeML Open RAIL-M licence beside it. That is what
;            makes this installer a complete image generator with nothing to fetch: SD 1.5 is the
;            checkpoint whose licence ALLOWS redistribution (RAIL-M, licence text included), where
;            SDXL Turbo does not (Stability's non-commercial research licence) and is therefore left
;            to tools\sd-cpu\FETCH_IMAGE_MODEL.bat.
...
#define Payload "D:\LYGO_CANON\2026-09-24_build-1.3.2_PC_LOCAL\PC_LOCAL"
...
AppComments=One console, three systems: USB LOCAL / PC LOCAL / WEB PORTAL (API only). Sealed 1.3.2 build with everything inside: the chat engine, the picture engine (stable-diffusion.cpp), gemma4-12b (text + images + audio), its projector, the qwen2.5-coder, the embedding model, and the SD 1.5 image checkpoint. Install it and draw a picture - nothing to fetch.
...
VersionInfoDescription=LYGO Local Agent Console (PC LOCAL) 1.3.2 FULL - sealed build + picture engine + gemma4-12b + coder + embeddings + SD 1.5 checkpoint inside
...
OutputDir=D:\
OutputBaseFilename=LYGO_LLM_CONSOLE_1.3.2_PC_SETUP_FULL
...
InfoAfterFile=C:\Users\justi\AppData\Local\Temp\lygo_installer\INSTALL_NOTES_1.3.2_PC_FULL.txt
...
[Icons]
Name: "{autoprograms}\LYGO Local Agent Console\LYGO Local Agent Console (PC LOCAL)"; Filename: "{app}\LYGO_LLM_CONSOLE.bat"; WorkingDir: "{app}"; Comment: "Boot the PC LOCAL console (ports 9641 / 11441) - brains and picture engine included"
...
begin
  if CurStep = ssPostInstall then
    Log('LYGO PC LOCAL FULL ' + '{#Release}' + ' installed (brains + picture engine + checkpoint inside) to ' + ExpandConstant('{app}'));
end;
