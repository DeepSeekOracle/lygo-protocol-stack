; ---------------------------------------------------------------------------
;  LYGO Local Agent Console - PC LOCAL - FULL installer, 1.5.5  (everything inside)
;
;  Payload : the SEALED 1.5.5 build  D:\LYGO_CANON\2026-09-25_build-1.5.5_PC_LOCAL\PC_LOCAL
...
AppComments=One console, three systems: USB LOCAL / PC LOCAL / WEB PORTAL (API only). Sealed 1.5.5 build with everything inside: the chat engine, the picture engine (stable-diffusion.cpp), gemma4-12b, its projector, the qwen2.5-coder, the embedding model, and the SD 1.5 image checkpoint. Install it and draw a picture - nothing to fetch. 1.5.5 adds the music systems: the exact YuE-shaped lyric template is the box's default (one press restores it), the sheet is validated and converted without spending a render, and the agent can write a whole song into the box from a brief or at random from the engine's own tag vocabulary.
...
VersionInfoDescription=LYGO Local Agent Console (PC LOCAL) 1.5.5 FULL - sealed build + picture engine + gemma4-12b + coder + embeddings + SD 1.5 checkpoint inside
...
OutputDir=D:\
OutputBaseFilename=LYGO_LLM_CONSOLE_1.5.5_PC_SETUP_FULL
