; ---------------------------------------------------------------------------
;  LYGO Local Agent Console - USB CLAW / stick build - installer, 1.5.3
;
;  Payload : the SEALED 1.5.3 stick build
;            D:\LYGO_CANON\2026-09-24_build-1.5.3_USB_CLAW\USB_CLAW
...
;            LYGO_LLM_CONSOLE_1.5.3_USB_SETUP_FULL.
...
AppComments=The stick build: own engine, own Python, own model CAS, and now its own picture engine (stable-diffusion.cpp, CPU build) inside the kit. One console, three systems: USB LOCAL / PC LOCAL / WEB PORTAL (API only). Sealed 1.5.3 - the picture engine travels with the console and finds itself on the machine you plug into; the checkpoint is one fetch. 1.5.3 adds the music systems: the exact YuE-shaped lyric template is the box's default (one press restores it), the sheet is validated and converted without spending a render, and the agent can write a whole song into the box from a brief or at random from the engine's own tag vocabulary.
AppCopyright=LYGO Sovereign License v3.0 - no resale, no rebranding, no modified redistribution.
VersionInfoVersion=1.5.3.0
VersionInfoDescription=LYGO Local Agent Console (USB CLAW) 1.5.3 - sealed stick build, own engine and Python, picture engine inside
...
OutputDir=D:\
OutputBaseFilename=LYGO_LLM_CONSOLE_1.5.3_USB_SETUP
...
begin
  if CurStep = ssPostInstall then
    Log('LYGO USB CLAW ' + '{#Release}' + ' installed from the sealed 1.5.3 stick build to ' + ExpandConstant('{app}') +
