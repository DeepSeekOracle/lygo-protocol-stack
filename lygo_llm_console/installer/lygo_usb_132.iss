; ---------------------------------------------------------------------------
;  LYGO Local Agent Console - USB CLAW / stick build - installer, 1.3.2
;
...
;            LYGO_LLM_CONSOLE_1.3.2_USB_SETUP_FULL.
...
AppComments=The stick build: own engine, own Python, own model CAS, and now its own picture engine (stable-diffusion.cpp, CPU build) inside the kit. One console, three systems: USB LOCAL / PC LOCAL / WEB PORTAL (API only). Sealed 1.3.2 - the picture engine travels with the console and finds itself on the machine you plug into; the checkpoint is one fetch.
AppCopyright=LYGO Sovereign License v3.0 - no resale, no rebranding, no modified redistribution.
VersionInfoVersion=1.3.2.0
VersionInfoDescription=LYGO Local Agent Console (USB CLAW) 1.3.2 - sealed stick build, own engine and Python, picture engine inside
...
OutputDir=E:\
OutputBaseFilename=LYGO_LLM_CONSOLE_1.3.2_USB_SETUP
...
InfoAfterFile=C:\Users\justi\AppData\Local\Temp\lygo_installer\INSTALL_NOTES_1.3.2_USB.txt
...
begin
  if CurStep = ssPostInstall then
    Log('LYGO USB CLAW ' + '{#Release}' + ' installed from the sealed 1.3.2 stick build to ' + ExpandConstant('{app}') +
