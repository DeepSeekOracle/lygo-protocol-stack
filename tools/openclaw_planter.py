#!/usr/bin/env python3
"""Plant lygo-openclaw-v10 kernel egg + public registry (consent-gated)."""
...
REGISTRY_PUBLIC = ROOT / "docs" / "OpenClawRegistry.json"
MANIFEST = ROOT / "data" / "openclaw" / "openclaw_egg_manifest.json"
SIGNATURE = "Δ9Φ963-OPENCLAW-EGG-v1"
...
def build_manifest() -> dict:
    ...
    manifest["artifacts"].append(
        {
            "label": "openclaw_egg_manifest",
            "path": "data/openclaw/openclaw_egg_manifest.json",
            "sha256": digest,
    ...
    registry = {
        ...
        "eggs": [{"egg_id": EGG_ID, "merkle_root": merkle_root, "manifest_path": "data/openclaw/openclaw_egg_manifest.json"}],
        "ledger": "data/openclaw/workflow_runs.jsonl",
    }
...
def main() -> int:
    ...
    args = ap.parse_args()
    if not args.i_consent and os.environ.get("LYGO_OPENCLAW_PLANT_CONSENT", "").lower() not in (
        "yes",
        "1",
        "true",
    ):
    ...
    sc = ROOT / "clawhub" / "mirrors" / "lygo-openclaw" / "scripts" / "self_check.py"
    if sc.is_file():
    ...
    print(f"[openclaw] planted {EGG_ID} merkle={reg['registry_merkle_root'][:16]}…")
    return 0
