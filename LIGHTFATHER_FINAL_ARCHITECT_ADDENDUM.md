...
| 3 | "Cure" → file-integrity honesty | `lygo-file-integrity-checker` on ClawHub; `lygo-universal-cure-system` deprecated → successor slug |
...
`lygo_p0_lyra_kernel.py` previously integrated `OathVectorEngine` with **fixed** ethics literals — not a measurable ethics gate. **Biophase7:** `OathVectorEngine` **removed**; structural `LYGOValidator` bounds only.
