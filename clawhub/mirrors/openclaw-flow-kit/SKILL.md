...
### 2.1) MoltX post pipeline (engage-gate → post → backoff/retry → receipt)
```bash
python scripts/moltx_post_pipeline.py --text-file tmp/drafts/moltx/my_post.txt --random-seal
```
Outputs JSON including `post_id` and writes a receipt to `tmp/receipts/moltx/`.
...
### 3.1) Path drift linter (find common mistakes)
```bash
python scripts/path_drift_lint.py --root .
```
Flags `skills/state` writes and hardcoded workspace paths.
