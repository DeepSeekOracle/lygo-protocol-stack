...
async function pulseLiveLattice(pagesBase) {
  ...
  const feedEntries = Array.isArray(feed?.entries) ? feed.entries.slice(0, 5) : [];
...
async function verifyLocalStack(stackRoot) {
  const checks = { stack_root: stackRoot, exists: existsSync(stackRoot) };
  if (!checks.exists) {
  ...
  try {
    ...
    checks.local_registry_sha16 = createHash("sha256").update(raw).digest("hex").slice(0, 16);
  } catch {
    checks.local_registry_sha16 = false;
  }

  try {
    const skills = JSON.parse(readFileSync(path.join(stackRoot, "clawhub/skills.json"), "utf8"));
    checks.clawhub_skill_count = String(Array.isArray(skills.skills) ? skills.skills.length : 0);
  } catch {
...
function p0QuickScan(text) {
  ...
  if (!len) return { signature: SIGNATURE, verdict: "REJECT", score: 0, reasons: ["empty_payload"] };
