...
// src/index.ts
var PLUGIN_INSTALL = "openclaw plugins install clawhub:@deepseekoracle/lygo-continuum";
var PORTAL = "https://chatagent.ca/lygo-continuum.html";
...
var index_default = definePluginEntry({
  id: "lygo-continuum",
  name: "LYGO Continuum",
  description: "Falsifiable work capsules for OpenClaw agents: seal done-claims, re-verify, detect drift, handoff packs. Pure local \u2014 no network, no subprocess.",
  register(api) {
    ...
    api.registerTool({
      ...
      description: "Seal falsifiable claims into a Continuum capsule. Auto-fills file_sha256 expect when omitted. Returns capsule JSON with root_hash and sealed_results.",
      parameters: Type.Object({
      ...
      async execute(_id, params) {
        try {
          ...
          const decisions = params.decisions_json ? JSON.parse(String(params.decisions_json)) : [];
          const next_actions = params.next_actions_json ? JSON.parse(String(params.next_actions_json)) : [];
          const base = resolveBase(cfg(), params.base);
          const capsule = sealCapsule({
            claims,
    ...
    api.registerTool({
      ...
      description: "Re-verify a Continuum capsule against the current filesystem. Returns integrity_ok, claims_ok, drift, pass/fail.",
      parameters: Type.Object({
    ...
    api.registerTool({
      ...
      description: "Verify capsule and return a slim drift report (what changed since seal).",
      parameters: Type.Object({
        capsule_json: Type.Optional(Type.String()),
        capsule_path: Type.Optional(Type.String()),
        base: Type.Optional(Type.String())
    ...
    api.registerTool({
      ...
      description: "Emit a markdown handoff pack (claims + decisions + optional live verify + embedded capsule JSON) for the next agent or human.",
      parameters: Type.Object({
        capsule_json: Type.Optional(Type.String()),
        capsule_path: Type.Optional(Type.String()),
        base: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description: "BEFORE claiming work is done: seal claims and require all pass. Returns can_claim_done true only if sealed_ok. Agents must call this instead of asserting done in prose.",
      parameters: Type.Object({
        claims_json: Type.String(),
      ...
      async execute(_id, params) {
        try {
          ...
          return textResult({
            ...
            total: verify.total,
            fail_results: verify.results.filter(
              (r) => !r.ok
            ),
            capsule_id: capsule.id,
            root_hash: capsule.root_hash,
            capsule,
            policy: "If can_claim_done is false, do NOT tell the user the task is done. Fix files or claims first.",
            signature: SIG,
            portal: PORTAL
          });
        } catch (e) {
    ...
    api.registerTool({
      ...
      description: "Self-contained demo: seal \u2192 verify HOLDS \u2192 tamper \u2192 drift. No network. Returns ok:true if engine works.",
      parameters: Type.Object({}),
      async execute() {
